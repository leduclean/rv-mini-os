 ...
 * cat memccpy.c memchr.c memrchr.c memcmp.c memcpy.c memmove.c memset.c
 * strxspn.c memmem.c memswap.c strcat.c strchr.c strrchr.c strcmp.c strcpy.c
 * strcspn.c strlen.c strnlen.c strncat.c strlcat.c strncmp.c strncpy.c
 * strpbrk.c strsep.c strspn.c strstr.c strtok.c
 *
 * in the subdirectory :
 * usr/klibc/
 *
...
void *memmove(void *dst, const void *src, size_t n) {
  ...
  if (q < p) {
    __asm__ __volatile__("cld ; rep ; movsb" : "+c"(n), "+S"(p), "+D"(q));
  } else {
    p += (n - 1);
    q += (n - 1);
    __asm__ __volatile__("std ; rep ; movsb" : "+c"(n), "+S"(p), "+D"(q));
  }
#else
  if (q < p) {
    while (n--) {
