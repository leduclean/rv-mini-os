...
static void _check_split(block_t *block, unsigned requested_size)
{
	...
	if (unused >= MIN_BLOCK_SIZE) {
		// Split
		block_t *split =
			(block_t *)((char *)(block + 1) + requested_size);
		_init_nodes(split);
		split->size = unused;
		split->free = 1;
...
{
	size = ALIGN(size);
	clist_node_t *found_node =
		clist_find(&free_list, _sufficient_size, &size);
...
static block_t *_check_merge(block_t *block)
{
	...
	if (current_node->next->in_list) {
		block_t *next =
			container_of(current_node->next, block_t, memory_node);
		if (next->free) {
	...
	if (current_node->prev->in_list) {
		block_t *prev =
			container_of(current_node->prev, block_t, memory_node);
		if (prev->free) {
			merged = (merged) ? _merge(prev, merged) :
					    _merge(prev, block);
		}
	}
