...
int UnityGetCommandLineOptions(int argc, const char *argv[])
{
	...
	for (i = 1; i < argc;) {
		if (strcmp(argv[i], "-h") == 0 ||
			...
			UNITY_PRINT_EOL();
			UnityPrint(
				"When no flag is specified, all tests are run.");
			UNITY_PRINT_EOL();
			UNITY_PRINT_EOL();
			UnityPrint("Optional flags:");
			UNITY_PRINT_EOL();
			UnityPrint(
				"  -v          Verbose output: show all tests executed even "
				"if they pass");
			UNITY_PRINT_EOL();
			UnityPrint(
				"  -s          Silent mode: minimal output showing only test "
				"failures");
			UNITY_PRINT_EOL();
			UnityPrint("  -d          Dry run all tests");
			UNITY_PRINT_EOL();
			UnityPrint(
				"  -g NAME     Only run tests in groups that contain the "
				"string NAME");
			UNITY_PRINT_EOL();
			UnityPrint(
				"  -G NAME     Only run tests in groups named NAME");
			UNITY_PRINT_EOL();
			UnityPrint(
			...
			/* User-defined help message, e.g. to point to project-specific
       * documentation */
